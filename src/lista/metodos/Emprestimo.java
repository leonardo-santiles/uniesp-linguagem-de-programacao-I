...
public class Emprestimo {
  ...
  public void calcularEmprestimo() {
    ...
    if (idade >= 18 && idade < 23) {
      ...
      BigDecimal valor = new BigDecimal(this.valorEmprestimo).multiply(percentualJurosMes)
          .divide(mesesParaPagar, 2, RoundingMode.HALF_UP);

      this.valorEmprestimo = valor.toString();
    }

    if (idade > 23 && idade <= 33) {
      BigDecimal valor = new BigDecimal(this.valorEmprestimo).multiply(DEZ)
          .divide(CEM, 2, RoundingMode.HALF_UP);

      this.valorEmprestimo = valor.toString();
    }

    System.out.println("Valor do empréstimo: " + valorEmprestimo.toString());
  }
}
